...
A.extend({
	...
	,formateString:function(str,data){
		...
		if(data instanceof Array){
			...
			//搜索{#key#}格式化字符串，并在data中查找对应的key属性替换
...
A.fn.extend({
	...
	,html:function(){
		...
		//如果两个参数,且第二个参数值为true，则为获取到的所有元素追加内容
		}else if(len === 2 && arg[1]){
			for(var i =this.length - 1;i>=0;i--){
				this[i].innerHTML += arg[0];
			}
...
var Tween = {
	...
	endFn:function(instance){
		instance.passed = instance.durtion;
		instance.tween = instance.to;
		this.exec(instance);
		this.destory(instance);
	},
	/***
	 *@name  执行主函数
	 *@param instance  运动成员
