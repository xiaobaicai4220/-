 ...
 *选择符Q
 *接口
 *Q.extend(Q,object)：在Q的内部作用域中扩展
 *Q.extend(Q.fn,object)：扩展Q的实例化对象
 *Q.attr:调用Q内部作用域中的属性
 *Q(selector).attr:调用Q实例化对象作用域中的属性
 **/
...
/***	Q属性扩展    ***
 *Q.fn.on(type,fn):绑定事件
 *Q.camelCase(str):"-"转驼峰
 *Q.fn.css(object):设置css样式
 *Q.fn.attr(object):设置属性
 *Q.fn.html(str):设置innerHTML
 ***/
			...
			//为对象时则设置多个样式
				...
				for(var i in arg[0]){	//遍历每个样式
					for(var j = this.length - 1;j>=0;j--){
						...
						this[j].style[Q.camelCase(i)] = arg[0][i];
					}
				}
			}
		//两个参数则设置一个样式
			...
			for(var j = this.length-1;j>= 0;j--){
				this[j].style[Q.camelCase(arg[0])] = arg[1];
			}
		}
		return this;
	}
